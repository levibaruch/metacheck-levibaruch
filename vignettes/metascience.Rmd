...
training_features_log <- text_features(
  text = ground_truth$text, 
  words = words,
  has_number = TRUE,
...
# show the first row
training_features_log[1, ] |> str()
```

You can then use this coded data to train a model. Here, we're using a simple binomial logistic regression to predict the classification from all fo the features.

```{r}
# Train logistic regression model
power_log_model <- glm(ground_truth$power_computation ~ .,
                 data = training_features_log,
                 family = "binomial")

summary(power_log_model)
```
...
Now you can classify any text using this model. Here, we're going to predict the classification of the original training data. Use `predict_classification()` to return the model response, and compare this result to a threshold (here 0.5) to generate the predicted classification.

```{r}
ground_truth <- ground_truth |>
  mutate(
    model_response = predict_classification(
      power_log_model, text, words, TRUE, has_symbols),
    power_computation_predict = model_response > 0.5
  )
...
```{r}
saveRDS(power_log_model, "power/power_log_model.Rds")
```
...
table$model_response <- papercheck::predict_classification(
  power_log_model, table$text, words, TRUE, has_symbols
)
